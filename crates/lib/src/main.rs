...
fn main() -> Result<(), Whatever> {
    ...
    use the_blockheads_tools_lib::game::db::world_db::WorldDb;
    let world_db = WorldDb::from_path(db_path);
    if let Err(e) = world_db {
        let e = snafu::Report::from_error(e);
        println!("{}", e);
        return Ok(());
    }
    let world_db = world_db.unwrap();

    dbg!(world_db.main.dynamic_world_v2.dynamic_object_id_count);
