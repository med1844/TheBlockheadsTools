...
#[cfg(test)]
mod tests {
    use super::{super::DynamicObjectList, Blockhead, Inventory};
...
<array>
        ...
        let round_trip_inventory_data = to_xml_plist(&inventory).expect("should serialize");
        ...
        let inventory_value: plist::Value =
            plist::from_reader_xml(inventory_data.as_slice()).expect("should deserialize");
