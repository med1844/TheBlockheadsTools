...
pub const MAX_SIZE: egui::Vec2 =
    egui::Vec2::new(GRID_COLS as f32 * COL_PX, GRID_ROWS as f32 * ROW_PX);

#[repr(C)]
#[derive(Copy, Clone, Debug, bytemuck::Pod, bytemuck::Zeroable)]
pub struct ItemSelectorUniforms {
...
pub struct ItemSelectorRenderer {
...
impl ItemSelectorRenderer {
    pub fn new(
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Item Selector Shader"),
            source: wgpu::ShaderSource::Wgsl(
                wgsl_macro::include_wgsl!("src/render/shader/item_selector.wgsl").into(),
            ),
        });
        ...
        let bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            ...
            label: Some("item_selector_bind_group_layout"),
        });

        let uniform_buf = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("Item Selector Uniforms Buffer"),
            size: std::mem::size_of::<ItemSelectorUniforms>() as wgpu::BufferAddress,
        ...
        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            label: Some("item_selector_bind_group"),
        });

        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("Item Selector Pipeline Layout"),
            bind_group_layouts: &[&bind_group_layout],
            push_constant_ranges: &[],
        });

        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("Item Selector Render Pipeline"),
        ...
        let instances: Vec<DwItemInstanceRaw> = ItemType::iter()
            ...
            .map(|(i, item)| {
                ...
                DwItem::selector_instance(col, row, item)
            })
            .collect();
    ...
    pub fn prepare(
        ...
        let uniforms = ItemSelectorUniforms {
