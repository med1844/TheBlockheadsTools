use clap::Parser;
use lmdb_rs::constants::{P_BRANCH, P_LEAF, P_META, P_OVERFLOW};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if free_db.root_page != u64::MAX && free_db.entries > 0 {
        // u64::MAX or 0?
        // Root page might be valid even if empty?
        let env_data = &mmap[..];
        let mut cursor =
            lmdb_rs::cursor::Cursor::new(env_data, active_arch, free_db.root_page, page_size);
    ...
    while pgno <= last_page {
        ...
        if pgno == 346 {
            println!(
                "    [DEBUG] Page 346 Header: {}",
                hex_dump(&page_data[0..16])
            );
        }
        ...
        match Page::new(page_data, active_arch) {
            Ok(page) => {
                ...
                if is_free {
                    println!(
                        "Page {}: [FREE] Parsed as {} (Flags: {:#x})",
                        pgno,
                        type_str,
                        page.flags(active_arch)
                    );
                } else {
                    // Normal Print
                    match &page {
