...
impl<'a> Cursor<'a> {
    ...
    fn get_page(&self, pgno: u64) -> Result<Page<'a>> {
        ...
        if offset + page_size > self.data.len() {
             return Err(crate::error::Error::UnexpectedEof { expected: offset + page_size, available: self.data.len() });
        }
    ...
    pub fn get(&mut self, key: &[u8]) -> Result<Option<&'a [u8]>> {
        ...
        loop {
            ...
            match page {
                Page::Leaf(leaf) => {
                    match leaf.search(key) {
                        Ok(idx) => {
                            ...
                            // Handle Value
                            if node.is_bigdata() {
                                let overflow_pgno = node.overflow_page_number()
                                    .ok_or(crate::error::Error::CorruptedTree { message: "Invalid overflow node" })?;
                                ...
                                if let Page::Overflow(_) = overflow_page {
                                     ...
                                     let start_offset = overflow_pgno as usize * self.page_size + header_sz;
                                     let total_len = node.data_size();
                                     
                                     if start_offset + total_len > self.data.len() {
                                         return Err(crate::error::Error::UnexpectedEof { expected: start_offset + total_len, available: self.data.len() });
                                     }
                                     return Ok(Some(&self.data[start_offset..start_offset+total_len]));
                                } else {
                                     return Err(crate::error::Error::CorruptedTree { message: "BigData node pointed to non-overflow page" });
                                }
                            } else {
                ...
                _ => return Err(crate::error::Error::CorruptedTree { message: "Unexpected page type during traversal" }),
            }
        }
    }
    ...
    fn descend_left(&mut self) -> Result<()> {
        loop {
             ...
             match page {
                 ...
                 Page::Branch(branch) => {
                      if branch.num_keys() == 0 { return Ok(()); }
                      let node = branch.get_node(0)?;
                      self.stack.push((node.child_page_number(), 0));
                 },
                 _ => return Err(crate::error::Error::CorruptedTree { message: "Unexpected page type" }),
             }
        }
    }

    pub fn get_current(&mut self) -> Result<Option<(&'a [u8], &'a [u8])>> {
         if self.stack.is_empty() { return Ok(None); }
         let (pgno, idx) = *self.stack.last().unwrap();
         let page = self.get_page(pgno)?;
         
         match page {
             Page::Leaf(leaf) => {
                 if idx >= leaf.num_keys() { return Ok(None); }
                 let node = leaf.get_node(idx)?;
                 ...
                 let val_slice = if node.is_bigdata() {
                      ...
                      let overflow_pgno = node.overflow_page_number()
                          .ok_or(crate::error::Error::CorruptedTree { message: "Invalid overflow node" })?; // Modified this line
                      let total_len = node.data_size();
                      ...
                      node.val_data().ok_or(crate::error::Error::UnexpectedEof { expected: 0, available: 0 })?
                 };
    ...
    pub fn iter_start(&mut self) -> Result<CursorIter<'_, 'a>> {
        self.to_first()?; 
        Ok(CursorIter { cursor: self, should_advance: false })
    }

    pub fn iter_start_owned(mut self) -> Result<OwnedCursorIter<'a>> {
        self.to_first()?; 
        Ok(OwnedCursorIter { cursor: self, should_advance: false })
    }
    ...
    pub fn iter(&mut self) -> CursorIter<'_, 'a> {
         CursorIter { cursor: self, should_advance: false }
    }
}
...
mod tests {
    use super::*;
    use crate::arch::DynArch;
    use crate::constants::{P_LEAF, P_BRANCH, P_OVERFLOW};
    ...
    fn test_get_overflow_value() {
        ...
        buf[off1+node_off+11..off1+node_off+19].copy_from_slice(&overflow_pgno.to_le_bytes());
    ...
    fn test_iter_multi() {
        ...
        let l2_n0 = 100; write_leaf_node(&mut buf[off2..], l2_n0, b"a", b"v1");
        write_ptr(&mut buf[off2..], 0, l2_n0 as u16);
        
        let l2_n1 = 200; write_leaf_node(&mut buf[off2..], l2_n1, b"b", b"v2");
        ...
        let l3_n0 = 100; write_leaf_node(&mut buf[off3..], l3_n0, b"c", b"v3");
        write_ptr(&mut buf[off3..], 0, l3_n0 as u16);
        
        let l3_n1 = 200; write_leaf_node(&mut buf[off3..], l3_n1, b"d", b"v4");
    ...
    fn test_iter_range() {
        ...
        let n0 = 100; write_leaf_node(&mut buf[off..], n0, b"a", b"1"); write_ptr(&mut buf[off..], 0, n0 as u16);
        let n1 = 150; write_leaf_node(&mut buf[off..], n1, b"b", b"2"); write_ptr(&mut buf[off..], 1, n1 as u16);
        let n2 = 200; write_leaf_node(&mut buf[off..], n2, b"c", b"3"); write_ptr(&mut buf[off..], 2, n2 as u16);
        let n3 = 250; write_leaf_node(&mut buf[off..], n3, b"d", b"4"); write_ptr(&mut buf[off..], 3, n3 as u16);
    ...
    fn test_list_dbs() {
        ...
        let n0 = 100; write_leaf_node(&mut buf[off..], n0, b"db1", &db1_bytes);
        write_ptr(&mut buf[off..], 0, n0 as u16);
        
        // Key 2: "db2"
        let mut db2_bytes = vec![0u8; 48];
        db2_bytes[40..48].copy_from_slice(&200u64.to_le_bytes()); // root=200
        let n1 = 200; write_leaf_node(&mut buf[off..], n1, b"db2", &db2_bytes);
        write_ptr(&mut buf[off..], 1, n1 as u16);
        
        let mut cursor = Cursor::new(&buf, arch, root as u64, page_size);
        
        let dbs = cursor.list_dbs().unwrap();
        assert!(dbs.iter().any(|(name, db)| name == "db1" && db.root_page == 100));
        assert_eq!(dbs.len(), 2);
    }
    ...
    fn test_get_overflow_value_32() {
        ...
        buf[off1+node_off+11..off1+node_off+15].copy_from_slice(&overflow_pgno.to_le_bytes());
