...
impl<'a, A: Arch> LeafPageBuilder<'a, A> {
    ...
    pub fn push_overflow(&mut self, key: &'a [u8], full_size: usize, overflow_pgno: u64) -> bool {
        ...
        self.push_internal(key, EntryContent::Owned(A::PGNO_SIZE, pgno_bytes), full_size, crate::constants::F_BIGDATA)
    }

    fn push_internal(&mut self, key: &'a [u8], content: EntryContent<'a>, logical_size: usize, flags: u16) -> bool {
        ...
        let used_bottom: usize = self.entries.iter()
            .map(|(k, v, _, _)| {
                 self.calculate_node_size(k.len(), v.len())
            })
            .sum::<usize>() + node_phys_size;
    ...
    pub fn build(mut self, page_number: u64) -> Vec<u8> {
        ...
        let total_data_size: usize = self.entries.iter()
        ...
        for (i, (key, value, logical_size, entry_flags)) in self.entries.iter().enumerate() {
            ...
            match value {
                EntryContent::Ref(s) => self.buffer[d_start..d_start+phys_dsize].copy_from_slice(s),
                EntryContent::Owned(len, buf) => self.buffer[d_start..d_start+phys_dsize].copy_from_slice(&buf[0..*len]),
            }
...
impl<'a, A: Arch> BranchPageBuilder<'a, A> {
    ...
    fn calculate_node_size(&self, ksize: usize) -> usize {
        ...
        if !raw_size.is_multiple_of(2) { raw_size + 1 } else { raw_size }
    }
