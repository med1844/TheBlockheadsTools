...
impl<'a> OverflowReader<'a> {
    pub fn new(
        ...
        if start_offset >= env_data.len() {
            return Err(Error::UnexpectedEof { expected: start_offset + 16, available: env_data.len() });
        }
        
        // Verify header of first page
        let header_slice = &env_data[start_offset..];
        if header_slice.len() < 16 { // Min header size
             return Err(Error::UnexpectedEof { expected: 16, available: header_slice.len() });
        }
        ...
        if (flags & P_OVERFLOW) == 0 {
             return Err(Error::InvalidPageType { expected: P_OVERFLOW, found: flags });
        }
...
mod tests {
    ...
    fn test_overflow_read() {
        ...
        let reader = OverflowReader::new(&buf, page_size, start_page as u64, data_len, arch).unwrap();
    ...
    fn test_overflow_span_reading() {
        ...
        let reader = OverflowReader::new(&buf, page_size, start_page as u64, 4096*3, arch).unwrap();
